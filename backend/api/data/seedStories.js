...
// ✅ UNIQUE IMAGE (no repetition)
const generateUniqueImage = () => {
  const seed = faker.string.uuid(); // unique every time
  const width = faker.number.int({ min: 400, max: 600 });
  const height = faker.number.int({ min: 700, max: 900 });

  return `https://picsum.photos/seed/${seed}/${width}/${height}`;
};

// ✅ VIDEO STRATEGY
// 🔥 BEST: Replace with your Cloudinary videos
...
// pick video
const getVideo = () => {
  if (CLOUDINARY_VIDEOS.length > 0) {
    return faker.helpers.arrayElement(CLOUDINARY_VIDEOS);
  }
  return faker.helpers.arrayElement(FALLBACK_VIDEOS);
};
  ...
  for (const user of users) {
    for (let i = 0; i < STORIES_PER_USER; i++) {
      ...
      stories.push({
        ...
        media_url: isVideo
          ? getVideo()
          : generateUniqueImage(), // 🔥 UNIQUE EVERY TIME
